...
class JigsawPuzzle extends Emitter {
  ...
  hitTest(piece, px, py) {
    ...
    // The piece's offscreen canvas is drawn at (piece.x - piece.pad, piece.y - piece.pad) in world space.
    // The path inside the offscreen canvas starts at (piece.ox, piece.oy) = (pad+tabSize, pad+tabSize).
    // So a world point (wx, wy) maps to offscreen canvas coords:
    //   canvasX = (wx - (piece.x - piece.pad)) = wx - piece.x + piece.pad
    //   canvasY = (wy - (piece.y - piece.pad)) = wy - piece.y + piece.pad
    // And the path was built starting at (0,0) in local coords, then translated by (ox, oy) when drawn.
    // Since the path object itself is in unstranslated local coords, we need to check:
    //   isPointInPath(path, canvasX - piece.ox + piece.pad, canvasY - piece.oy + piece.pad)
    // But piece.ox = pad + tabSize, so: canvasX - (pad + tabSize) + pad = canvasX - tabSize
    // Simpler: just do a bounding box test first, then path test in canvas coords.

    // Fast bounding box check (in world space, accounting for pad)
    const left = piece.x - piece.pad;
    const top = piece.y - piece.pad;
    const w = piece.canvas ? piece.canvas.width : piece.width;
    const h = piece.canvas ? piece.canvas.height : piece.height;

    if (wx < left || wx > left + w || wy < top || wy > top + h) {
      return false;
    }

    // Path test: convert to offscreen canvas local coords
    // Canvas is drawn at (piece.x - piece.pad, piece.y - piece.pad)
    // Path was translated by (piece.ox, piece.oy) when drawn
    const canvasX = wx - (piece.x - piece.pad);
    const canvasY = wy - (piece.y - piece.pad);
    // Path coordinates are local starting at (0,0), drawn with translate(ox, oy)
    // So a point at canvas (canvasX, canvasY) is at path coords (canvasX - ox, canvasY - oy)
    const pathX = canvasX - piece.ox;
    const pathY = canvasY - piece.oy;

    if (!this._hitCtx) {
      this._hitCtx = document.createElement('canvas').getContext('2d');
    }
    return this._hitCtx.isPointInPath(piece.path, pathX, pathY);
  }
  ...
  shuffle() {
    ...
    // Scatter pieces in a moderate ring around the board — close enough to see
    const scatterRange = Math.max(boardW, boardH) * 0.2;
    ...
    if (this._board) {
      ...
      let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
      for (const piece of this.pieces) {
        const halfW = (piece.canvas?.width || piece.width) / 2;
        const halfH = (piece.canvas?.height || piece.height) / 2;
      ...
      this.offsetX = cw / 2 - (minX + boundsW / 2) * this.scale;
      this.offsetY = ch / 2 - (minY + boundsH / 2) * this.scale;
    }
    this._updateProgress();
    this.render();
  }
